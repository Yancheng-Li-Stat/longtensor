...
parse_formula <- function(fo, tib) {
  ...
  if(length(rhs_terms)==1){
    stop("Your input formula \n\n    ",
         deparse(fo),
         "\n\n only have two terms on the right hand side:\n\n    ",
         paste0(rhs_terms,sep="\n    "),
         "\nwhich cannot be transferred as a tensor or a matrix.")
  }
  ...
  if(length(rhs_terms)==2){
    stop("Your input formula \n\n    ",
         ...
         "\n\n only have two terms on the right hand side:\n\n    ",
         paste0(rhs_terms,sep="\n    "),
         "\nwhich cannot be transferred as tensor with dimension >= 3. You might want to use the longpca package instead ().")
  }
